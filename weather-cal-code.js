...
async function makeWidget(settings, name, iCloudInUse) {
  ...
  async function date(column) {
    ...
    if (dateSettings.staticDateSize == "small" || (dateSettings.dynamicDateSize && data.events.eventsAreVisible)) {
      ...
      if (dateSettings.jalalianDate && dateSettings.jalalianDate_eng) {
        df.locale = "fa"
        let dateText = provideText(ProvideJalalianDate(df.string(currentDate), currentDate), 
          dateStack, textFormat.smallDate)
      } else if (dateSettings.jalalianDate && !dateSettings.jalalianDate_eng) {
        df.locale = "fa"
    ...
    // Otherwise, show the large date.
      ...
      if (dateSettings.jalalianDate && dateSettings.jalalianDate_eng) {
        df.locale = "fa"
        let dateOne = provideText(ProvideJalalianDate(df.string(currentDate), currentDate), 
          dateOneStack, textFormat.largeDate1)
      } else if (dateSettings.jalalianDate && !dateSettings.jalalianDate_eng) {
        df.locale = "fa"
        let dateOne = provideText(df.string(currentDate), dateOneStack, textFormat.largeDate1)
      } else {
      ...
      if (dateSettings.jalalianDate && dateSettings.jalalianDate_eng) {
        df.locale = "fa"
        let dateTwo = provideText(ProvideJalalianDate(df.string(currentDate), currentDate), 
        dateTwoStack, textFormat.largeDate2)
      } else if (dateSettings.jalalianDate && !dateSettings.jalalianDate_eng) {
        df.locale = "fa"
        let dateTwo = provideText(df.string(currentDate), dateTwoStack, textFormat.largeDate2)
      } else {
  ...
  // Change all Persian and Arabic number to English
  function fixAnyNumbers(str) {
  ...
  // Provide JalalianDate = Persian Date in English
  function ProvideJalalianDate(textDate, currentDate) {
    // Weekdays and Month array in persian and english
    var englishWeekDays = ['Sunday','Monday','Tuesday','Wednesday',
        'Thursday','Friday','Saturday']
    var englishMonths = ["Farvardin", "Ordibehesht", "Khordad", "Tir", "Mordad", 
        "Shahrivar", "Mehr", "Aban", "Azar", "Dey", "Bahman", "Esfand"]
    let persianWeekDays = ["یکشنبه", "دوشنبه", "سه‌شنبه", 
        "چهارشنبه", "پنج‌شنبه", "جمعه", "شنبه" ]
    let persianMonth = ["فروردين", "ارديبهشت", "خرداد", "تير", "مرداد", "شهريور", 
        "مهر", "آبان","آذر", "دي", "بهمن", "اسفند"]

    // Get the number day of the week
    let dayOftheWeek = currentDate.getDay()

    // Get the number month (only) of the year
    let df = new DateFormatter()
    df.locale = "fa"
    df.dateFormat = "MM"
    let monthOftheYear = parseInt(fixAnyNumbers(df.string(currentDate)))

    // Change all Persian and Arabic number to English
    textDate = fixAnyNumbers(textDate)

    // Change all Persian Text into English
    textDate = textDate.replace(persianWeekDays[dayOftheWeek], 
        englishWeekDays[dayOftheWeek])
    textDate = textDate.replace(persianMonth[monthOftheYear - 1], 
        englishMonths[monthOftheYear - 1])
    
    // Hard Code, incase if necessary, Change all Persian Text into English
    /*
    textDate = textDate.replace("یکشنبه", englishWeekDays[dayOftheWeek])
    textDate = textDate.replace("دوشنبه", englishWeekDays[dayOftheWeek])
    textDate = textDate.replace("سه‌شنبه", englishWeekDays[dayOftheWeek])
    textDate = textDate.replace("چهارشنبه", englishWeekDays[dayOftheWeek])
    textDate = textDate.replace("پنج‌شنبه", englishWeekDays[dayOftheWeek])
    textDate = textDate.replace("جمعه", englishWeekDays[dayOftheWeek])
    textDate = textDate.replace("شنبه", englishWeekDays[dayOftheWeek])

    textDate = textDate.replace("فروردين", englishMonths[monthOftheYear - 1])
    textDate = textDate.replace("ارديبهشت", englishMonths[monthOftheYear - 1])
    textDate = textDate.replace("خرداد", englishMonths[monthOftheYear - 1])
    textDate = textDate.replace("تير", englishMonths[monthOftheYear - 1])
    textDate = textDate.replace("مرداد", englishMonths[monthOftheYear - 1])
    textDate = textDate.replace("شهريور", englishMonths[monthOftheYear - 1])
    textDate = textDate.replace("مهر", englishMonths[monthOftheYear - 1])
    textDate = textDate.replace("آبان", englishMonths[monthOftheYear - 1])
    textDate = textDate.replace("آذر", englishMonths[monthOftheYear - 1])
    textDate = textDate.replace("دي", englishMonths[monthOftheYear - 1])
    textDate = textDate.replace("بهمن", englishMonths[monthOftheYear - 1])
    textDate = textDate.replace("اسفند", englishMonths[monthOftheYear - 1])
    */
    return textDate
  }
